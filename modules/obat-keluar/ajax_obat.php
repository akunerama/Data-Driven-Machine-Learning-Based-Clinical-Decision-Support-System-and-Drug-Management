...
if (isset($_POST['dataidobat'])) {
   ...
   $batch_details = [];
    ...
    // Urutkan batch berdasarkan expired date (yang paling dekat di atas)
    usort($batch_details, function($a, $b) {
        return strtotime($a['expired_date']) - strtotime($b['expired_date']);
    });
    ...
    if (!empty($batch_details)) {
        ...
        $response['stok_total'] = array_sum(array_column($batch_details, 'sisa_stok'));
    } else {
        $response['error'] = 'Tidak ada stok tersedia';
        $response['stok_total'] = 0;
        $response['expired_info'] = 'Stok Habis';
    }
